...
namespace WSDLMerge
{
    internal static class WSDLMerger
    {
        ...
        static Uri GetParentUriString(Uri uri)
        {
            return new Uri(uri.AbsoluteUri.Remove(uri.AbsoluteUri.Length - uri.Segments.Last().Length - uri.Query.Length));
        }
            ...
            out string importLocation )
        {
            if ( node is XmlElement )
            {
                ...
                if ( importElement.Attributes["schemaLocation"] != null )
                {
                    ...
                    {
                        importLocation = new Uri((GetParentUriString(new Uri(filename))), location).ToString();
                    }
